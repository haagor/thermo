...
void* e2_thread_run(void* args)
{
    ...
    for (int w = 0; w < sub_mat->nb_iter; w++) // nombre d'iteration verticale
    {
        ...
        e2_verticale_iter_impair(sub_mat->matrix, sub_mat->x, sub_mat->y, sub_mat->size,
...
void e2_run(int size, int nb_iter, int nb_thread, int print)
{
    ...
    e2_fill_thread_array(threads, sub_mat, matrice, n, N, nb_iter, 1 << (4 + size), nb_thread);
    //print_array(sub_mat, nb_thread);

    e2_iter(matrice, sub_mat, n, N, nb_iter, nb_thread, print);
